...
const ChartSettings: React.FC<ChartSettingsProps> = ({ onClose, items }) => {
  ...
  // Get current opacity from the first chart if only one exists
  useEffect(() => {
    const charts = items.filter(item => item.type === 'bar-chart' || item.type === 'pie-chart');
    if (charts.length === 1) {
      ...
      <Box sx={{ 
        ...
        <Box>
          ...
          <Slider
            ...
            disabled={items.filter(item => item.type === 'bar-chart' || item.type === 'pie-chart').length !== 1}
          />
        </Box>
      </Box>
    </Paper>
